...
file_path = myconfig.root_path+"/4_Mélanie/"
file_name = file_path+"liste_Factures_Mélanie_Elian_V2.xlsx"
...
all_data = pd.read_excel(file_name, sheet_name='Sheet0')
...
total_2024 = all_data[all_data[myconfig.field_date].dt.year == 2024]['paris_lutherie1_affluence_base100k'].sum()
all_data['paris_lutherie1_affluence_base10k'] = all_data['paris_lutherie1_affluence_base100k'] / total_2024 * 10000
...
if make_graph:
    ...
    plt.plot(all_data[myconfig.field_date], all_data['paris_lutherie1_affluence_base10k'], marker='o', linestyle='-', color='red', label='Number of Transactions')
    ...
    plt.tight_layout()
    plt.savefig(file_path+"paris_lutherie1.png")
    plt.close()
...
all_data = all_data.rename(columns={'paris_lutherie1_affluence_base10k': myconfig.field_y})

# add a column "y_value" with the value 
all_data[myconfig.field_seriesname]='paris_lutherie1_affluence_base10k'
...
all_data.to_csv(myconfig.opendata_path+'paris_lutherie_1.csv', index=False)

all_train_valid_data = all_data[all_data[myconfig.field_train_valid_test] != 'test']
all_train_valid_data.to_csv(myconfig.git_path+'paris_lutherie_1.csv', index=False)
