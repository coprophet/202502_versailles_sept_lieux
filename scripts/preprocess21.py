...
file_path = myconfig.root_path+"21_Fred/"
file_name = file_path+"Export Versailles_ELC.xlsx"
...
all_data = pd.read_excel(file_name, sheet_name='Feuil1')
...
# filter out outliers in the CA TTC column having a value greater than 1000000 or less than 0
all_data = all_data[all_data['CA TTC'] > 0]
all_data = all_data[all_data['CA TTC'] < 2500]
...
all_data['versailles_secondemain_ca_ttc_base_100k'] = all_data['CA TTC'] / total_2024 * 100000
# sort by date
...
if make_graph:
    ...
    plt.plot(all_data[myconfig.field_date], all_data['versailles_secondemain_ca_ttc_base_100k'], marker='o', linestyle='-', color='red', label='Total Revenue')
    ...
    plt.tight_layout()
    plt.savefig(file_path+"versailles_secondemain.png")
    plt.close()
...
all_data = all_data.rename(columns={'versailles_secondemain_ca_ttc_base_100k': myconfig.field_y})

# add a column "y_value" with the value 
all_data[myconfig.field_seriesname]='versailles_secondemain_ca_ttc_base_100k'
...
all_data.to_csv(myconfig.opendata_path+'versailles_secondemain.csv', index=False)

all_train_valid_data = all_data[all_data[myconfig.field_train_valid_test] != 'test']
all_train_valid_data.to_csv(myconfig.git_path+'versailles_secondemain.csv', index=False)
