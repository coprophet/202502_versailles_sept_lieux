...
file_path = myconfig.root_path+"/12_SeptLieux/"
file_name = file_path+"Sept Lieux Revenus 2024 base 100_ELC.xlsx"
...
all_data = pd.read_excel(file_name, sheet_name='Sheet1')
# filter out the rows with missing values in the DATES column
all_data[myconfig.field_date] = pd.to_datetime(all_data['Date'], format='%d/%m/%Y')
all_data = all_data.dropna(subset=[myconfig.field_date])

# sum the number of visitors for the year 2024
total_2024 = all_data[all_data[myconfig.field_date].dt.year == 2024]['Base 100'].sum()
print("total_2024: ", total_2024)
all_data['versailles_restaurant2_ca_ttc_base_100k'] = all_data['Base 100'] / total_2024 * 100000
...
if make_graph:
    ...
    plt.plot(all_data[myconfig.field_date], all_data['versailles_restaurant2_ca_ttc_base_100k'], marker='o', linestyle='-', color='red', label='Total Revenue')
