...
file_path = myconfig.root_path+"/7_ArnaudG/"
file_name = file_path+"journal ventes_ELC.xlsx"
...
total_2024 = all_data[all_data[myconfig.field_date].dt.year == 2024]['ca_ttc'].sum()
print("total_2024: ", total_2024)
all_data['producteurbio_sablesdolonne_ca_ttc_base_10k'] = all_data['ca_ttc'] / total_2024 * 10000
...
if make_graph:
    ...
    plt.plot(all_data[myconfig.field_date], all_data['producteurbio_sablesdolonne_ca_ttc_base_10k'], marker='o', linestyle='-', color='red', label='Total Revenue')
    ...
    plt.tight_layout()
    plt.savefig(file_path+"producteurbio_sablesdolonne.png")
    plt.close()
...
all_data = all_data.rename(columns={'producteurbio_sablesdolonne_ca_ttc_base_10k': myconfig.field_y})

# add a column "y_value" with the value 
all_data[myconfig.field_seriesname]='producteurbio_sablesdolonne_ca_ttc_base_10k'
...
all_data.to_csv(myconfig.opendata_path+'producteurbio_sablesdolonne.csv', index=False)

all_train_valid_data = all_data[all_data[myconfig.field_train_valid_test] != 'test']
all_train_valid_data.to_csv(myconfig.git_path+'producteurbio_sablesdolonne.csv', index=False)
