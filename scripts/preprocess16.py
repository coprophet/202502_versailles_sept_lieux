...
file_path = myconfig.root_path+"16_Elec/"
file_name = file_path+"bilan-electrique-jour.xlsx"
...
all_data = pd.read_excel(file_name, sheet_name='bilan-electrique-jour')
# filter out the rows with missing values in the DATES column
all_data[myconfig.field_date] = pd.to_datetime(all_data['jour'])

all_data = all_data[[myconfig.field_date, 'consommation_telerelevee_residentielle']]

# sum the number of visitors for the year 2024
total_2024 = all_data[all_data[myconfig.field_date].dt.year == 2024]['consommation_telerelevee_residentielle'].sum()

print("total_2024: ", total_2024)
all_data['consommation_elec_base100k'] = all_data['consommation_telerelevee_residentielle'] / total_2024 * 100000
# sort by date
...
if make_graph:
    ...
    plt.plot(all_data[myconfig.field_date], all_data['consommation_elec_base100k'], marker='o', linestyle='-', color='red', label='Total Revenue')
    ...
    plt.tight_layout()
    plt.savefig(file_path+"consommation_elec.png")
    plt.close()
...
all_data = all_data.rename(columns={'consommation_elec_base100k': myconfig.field_y})

# add a column "y_value" with the value 
all_data[myconfig.field_seriesname]='consommation_elec_base100k'
...
all_data.to_csv(myconfig.opendata_path+'consommation_elec.csv', index=False)

all_train_valid_data = all_data[all_data[myconfig.field_train_valid_test] != 'test']
all_train_valid_data.to_csv(myconfig.git_path+'consommation_elec.csv', index=False)
